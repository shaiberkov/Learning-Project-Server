...
public class EquationWithTwoUnknownsQuestion implements MathQuestion {
    ...
    public QuestionEntity generateQuestion(int difficulty) {
        ...
        String questionText;
        boolean isXFirst = random.nextBoolean(); // מיקום אקראי של x ו-y

        if (isXFirst) {
            questionText = String.format("%dx + %dy = %d\n%dx + %dy = %d", a1, b1, c1, a2, b2, c2);//TODO לעשות שירד שורה
        } else {
        ...
        return new QuestionEntity("מתמטיקה", "מערכת משוואות עם שני נעלמים", questionText, answer);
    }
